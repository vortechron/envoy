@setup
    ...
    $branch = isset($branch) ? $branch : "feature/vortechron-deployment";
    $path = rtrim($path, '/');
...
@task('deployment_composer')
    ...
    cd {{ $release }}
    echo "{{ $php }} {{ $composer }} install --no-interaction --quiet --no-dev --prefer-dist
    --optimize-autoloader"
    {{ $php }} {{ $composer }} install --no-interaction --quiet --no-dev --prefer-dist --optimize-autoloader
@endtask
